...
def makeBBoxString(bbox):
	return "lon{0[0]:.2f}_{0[2]:.2f}lat{0[1]:.2f}_{0[3]:.2f}".format(
		bbox)
...
def parseFileForBbox(fullFilename, corrx, corry):
	return parseHgtFilename(os.path.split(fullFilename)[1], corrx, corry)

def calcHgtArea(filenames, corrx, corry):
	bboxes = [parseFileForBbox(f, corrx, corry) for f in filenames]
...
class hgtFile:
	...
	def borders(self, corrx=0.0, corry=0.0):
		...
		"""
		return parseFileForBbox(self.fullFilename, corrx, corry)
